...
/* ============================================================
   ...
   ============================================================ */
const DELAY_IND=6000;           // ms entre cada MP individual (antiflood)
const DELAY_GRP=8000;           // ms entre cada grupo
const TENTATIVAS=4;             // retentativas quando o fórum recusa
const CAMPO_GRUPO='usergroup';  // campo do formulário de MP em grupo
let TOK={};                     // campos ocultos do formulário (renovados a cada envio)
let CAMPO_USER='username';      // detectado no próprio formulário (username ou username[])
const CHARSET=(document.characterSet||'UTF-8').toUpperCase();
...
const CG=[{v:'melhores',l:'[DIR] Melhores da Quinzena - VOTAÇÃO ABERTA!'},{v:'escala',l:'[DIR] Escala da Atividade Quinzenal'},{v:'bloqueadas',l:'[DIR] Promoções Bloqueadas'},{v:'desbloqueadas',l:'[DIR] Promoções Desbloqueadas'},{v:'atividade',l:'[DIR] Atividade Quinzenal'},{v:'atualizacao',l:'[DIR] Atualização de Escala de Funções'},{v:'avmensal',l:'[DIR] Avaliação Mensal da Especialização Intermediária'},{v:'reuniao',l:'[DIR] Reunião Geral'},{v:'coleta',l:'[DIR] Coleta de Horários'},{v:'abertura',l:'[DIR] Carta de Abertura de Análise'}];
...
const sanitizeNick=s=>String(s).replace(/[\u0000-\u001F\u007F]/g,'').replace(/\s+/g,' ').trim();
...
/* ============================================================
   NICKNAMES — separador único "/" + captura automática
   ============================================================ */
...
function renderPills(form){
  ...
  (NS[form]||[]).forEach(nick=>{
    ...
    box.appendChild(img);
    const nome=document.createElement('span');nome.className='pnm';nome.textContent=nick;
    const btn=document.createElement('button');btn.type='button';btn.className='prm';
...
[quote][color=white][font=Poppins]${mot}[/font][/color][/quote]
...
/* ============================================================
   SISTEMA DE ENVIO
   Renova os campos ocultos antes de CADA MP (são de uso único),
   codifica no charset do fórum e só confirma com retorno real.
   ============================================================ */

async function carregarTokens(){
  try{
    const r=await fetch('/privmsg?mode=post&folder=inbox&_='+Date.now(),{credentials:'include',cache:'no-store'});
...
function encCampo(s){
  s=String(s);
  if(CHARSET==='UTF-8')return encodeURIComponent(s);
  let out='';
  for(const ch of s){
...
const montarCorpo=obj=>Object.entries(obj).map(([k,v])=>encCampo(k)+'='+encCampo(v)).join('&');

function lerResposta(html){
  const doc=new DOMParser().parseFromString(html,'text/html');
  const txt=((doc.body&&doc.body.textContent)||'').replace(/\s+/g,' ').trim();
  if(/(mensagem.{0,40}foi enviada|enviada com sucesso|message has been sent)/i.test(txt))return{ok:true};
  if(/(muito rapidamente|t[ãa]o rapidamente|aguarde|flood|espere.{0,20}segundos|intervalo)/i.test(txt))return{ok:false,retry:true,msg:'Antiflood'};
  if(/(usu[áa]rio.{0,40}(n[ãa]o existe|n[ãa]o encontrado|inexistente)|n[ãa]o foi poss[ií]vel encontrar)/i.test(txt))return{ok:false,msg:'Nick não encontrado'};
  ...
  if(doc.querySelector('textarea[name="message"]'))return{ok:false,retry:true,msg:'Recusado pelo fórum'};
  return{ok:true};
}

async function postMP(dest,subject,message){
  await carregarTokens();
  const dados=Object.assign({folder:'inbox',mode:'post',post:'1'},TOK,{subject,message});
  if(dest.grupo!=null)dados[CAMPO_GRUPO]=String(dest.grupo);
  else dados[CAMPO_USER]=dest.nick;
  const r=await fetch('/privmsg',{
    method:'POST',
    headers:{'Content-Type':'application/x-www-form-urlencoded; charset='+CHARSET},
    credentials:'include',
    body:montarCorpo(dados)
  });
...
const sendMP=(nick,subject,message)=>comRetentativa(()=>postMP({nick},subject,message));
const sendMPGroup=(grupo,subject,message)=>comRetentativa(()=>postMP({grupo},subject,message));
...
// Envio INDIVIDUAL: um por um, MP personalizada por nick
async function send(f){
  ...
  try{
    ...
    const nks=(NS[f]||[]).map(sanitizeNick).filter(n=>{
      if(!n||vistos.has(norm(n)))return false;
      vistos.add(norm(n));return true;
    });
    initP(f,nks.length);
    let ok=0,er=0;
...
// Envio para GRUPOS: via usergroup, um grupo por vez
